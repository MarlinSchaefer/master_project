...
def preprocess(inp):
    ...
    fconv1 = FConv1D(256, 64, 2048)(inp)
    f_bn_1 = keras.layers.BatchNormalization()(fconv1)
    conv_1 = keras.layers.Conv1D(64, 16)(f_bn_1)
...
def compile_model(model):
    model.compile(loss=['mean_squared_error', 'categorical_crossentropy'], loss_weights=[1.0, 2.0], optimizer='adam', metrics={'Out_SNR': 'mape', 'Out_Bool': 'accuracy'})
    ...
    class CustomDataSet():
        ...
        def load_data(self):
            with h5py.File(self.file_path) as FILE:
                self.signals = FILE['signals']['data'][:12500]
                self.noise = FILE['noise']['data'][:37500]
                self.signal_labels = FILE['signals']['snr'][:12500]
