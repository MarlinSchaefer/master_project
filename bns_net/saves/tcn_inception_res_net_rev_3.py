...
def get_tcn_stack(inp, name="TCN"):
    ...
    for i in range(12):
        ...
        if i == 0:
            ...
            if i == 11:
                x = tcn_residual_block(x, k=kernel, d=2**i, name=name, num_filters=num_output_channels)
            else:
...
def get_model():
    ...
    tcn = get_tcn_stack(inp, name='TCN_part')
    add_tcn = keras.layers.Add()([inp, tcn])
    
    #Preprocessing
    batch_1 = keras.layers.BatchNormalization()(add_tcn)
    ...
    aux_out_1 = aux_out(res_3, 'Aux_Out_1')
    ...
    aux_out_2 = aux_out(res_6, 'Aux_Out_2')
    ...
    pool_2 = keras.layers.AveragePooling1D(4)(res_8)
    ...
    model = keras.models.Model(inputs=[in_sig, in_noi], outputs=[dense_2, dense_4, tcn, aux_out_1, aux_out_2])
    
    return(model)

def compile_model(model):
    model.compile(loss=['mean_squared_error', 'categorical_crossentropy', 'mean_squared_error', 'mse', 'mse'], loss_weights=[1.0, 0.5, 1.0, 0.3, 0.3], optimizer='adam', metrics={'Out_SNR': 'mape', 'Out_Bool': 'accuracy'})
