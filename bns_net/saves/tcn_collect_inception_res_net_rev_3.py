...
def get_model():
    ...
    tcn3 = get_tcn_stack(inp3, name="TCN_out_8s")
    tcn4 = get_tcn_stack(inp4, name="TCN_out_32s")
    ...
    pool_2 = keras.layers.MaxPooling1D(4)(lvl_3_1)
    ...
    model = keras.models.Model(inputs=[is1, in1, is2, in2, is3, in3, is4, in4], outputs=[dense_2, dense_4, tcn1, tcn2, tcn3, tcn4, aux_11, aux_12])
...
def compile_model(model):
    ...
    aux_weight = 0.1
    model.compile(loss=['mse', 'categorical_crossentropy', 'mse', 'mse', 'mse', 'mse', 'mse', 'mse'], loss_weights=[1.0, 0.5, aux_weight, aux_weight, aux_weight, aux_weight, aux_weight, aux_weight], optimizer='adam', metrics={'Out_SNR': 'mape', 'Out_Bool': 'accuracy'})
...
class DataGenerator(keras.utils.Sequence):
    ...
    def __data_generation(self, indices):
        ...
        num_inputs = 8
        X = [np.zeros([len(indices), num_detectors, 4096]) for i in range(num_inputs)]
        ...
        for i, idx in enumerate(indices):
            ...
            if not sig_ind == -1:
                X[0][i][0] = self.signals[sig_ind].transpose()[0]
                X[0][i][1] = self.signals[sig_ind].transpose()[7]
                X[2][i][0] = self.signals[sig_ind].transpose()[1]
                X[2][i][1] = self.signals[sig_ind].transpose()[8]
                X[4][i][0] = self.signals[sig_ind].transpose()[3]
                X[4][i][1] = self.signals[sig_ind].transpose()[10]
                X[6][i][0] = self.signals[sig_ind].transpose()[5]
                X[6][i][1] = self.signals[sig_ind].transpose()[12]
            
            X[1][i][0] = self.noise[noi_ind].transpose()[0]
            X[1][i][1] = self.noise[noi_ind].transpose()[7]
            X[3][i][0] = self.noise[noi_ind].transpose()[1]
            X[3][i][1] = self.noise[noi_ind].transpose()[8]
            X[5][i][0] = self.noise[noi_ind].transpose()[3]
            X[5][i][1] = self.noise[noi_ind].transpose()[10]
            X[7][i][0] = self.noise[noi_ind].transpose()[5]
            X[7][i][1] = self.noise[noi_ind].transpose()[12]
        ...
        return((X, [y_1, y_2, X[0], X[2], X[4], X[6], y_1, y_1]))
