...
def custom_softmax(x):
    shape = K.int_shape(x)
    print(shape)
    if not None in shape:
        print("Doing stuff")
        div = K.sum(x, axis=-1)
        div_mask = K.cast(div == 0., K.floatx())
        div = div * div_mask
        rand = np.random.randint(shape[-1], size=shape[0])
        overwrite_zero = np.zeros(shape)
        for r in rand:
            overwrite_zero[i][r] = 1.
        overwrite_zero = tf.convert_to_tensor(overwrite_zero)
        overwrite_zero = K.transpose(overwrite_zero)
        overwrite_zero = overwrite_zero * div_mask
        overwrite_zero = K.transpose(overwrite_zero)
        div = tf.convert_to_tensor(div_np)
        ret = K.transpose(x)
        ret = ret / div
        ret = K.transpose(ret)
        ret = ret + overwrite_zero
    else:
...
def get_model():
    ...
    inc1 = incp_lay(pool_2, 32)
    bn_1 = keras.layers.BatchNormalization()(inc1)
    inc2 = incp_lay(bn_1, 32)
    bn_2 = keras.layers.BatchNormalization()(inc2)
    res = keras.layers.Add()([bn_1, bn_2])
    
    dim_red = keras.layers.Conv1D(32, 1)(res)
    ...
    #out_bool = keras.layers.Lambda(custom_softmax, name='Out_Bool')(dense_4)
    
    model = keras.models.Model(inputs=[is1, in1, is2, in2, is3, in3, is4, in4, is5, in5, is6, in6, is7, in7, is8, in8], outputs=[dense_2, dense_4, tcn1, tcn2, tcn3, tcn4, tcn5, tcn6, tcn7, tcn8])
...
def compile_model(model):
    ...
    aux_weight = 0.1
    model.compile(loss=['mse', 'categorical_crossentropy', 'mse', 'mse', 'mse', 'mse', 'mse', 'mse', 'mse', 'mse'], loss_weights=[1.0, 0.5, aux_weight, aux_weight, aux_weight, aux_weight, aux_weight, aux_weight, aux_weight, aux_weight], optimizer='adam', metrics={'Out_SNR': 'mape', 'Out_Bool': 'accuracy'})
    ...
    class CustomDataSet():
        ...
        def load_data(self):
            with h5py.File(self.file_path) as FILE:
                self.signals = FILE['signals']['data'][:20000]
                self.noise = FILE['noise']['data'][:100000]
                self.signal_labels = FILE['signals']['snr'][:20000]
...
class DataGenerator(keras.utils.Sequence):
    ...
    def __data_generation(self, indices):
        ...
        return((X, [y_1, y_2, X[0], X[2], X[4], X[6], X[8], X[10], X[12], X[14]]))
