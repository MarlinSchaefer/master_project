...
def get_model():
    ...
    model = keras.models.Model(inputs=[in_sig, in_noi], outputs=[dense_2, dense_4, tcn])
    
    return(model)

def compile_model(model):
    model.compile(loss=['mean_squared_error', 'categorical_crossentropy', 'mean_squared_error'], loss_weights=[1.0, 0.5, 1.0], optimizer='adam', metrics={'Out_SNR': 'mape', 'Out_Bool': 'accuracy', 'TCN_part': 'mape'})
