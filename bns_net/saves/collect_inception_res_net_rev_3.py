...
def compile_model(model):
    model.compile(loss=['mean_squared_error', 'categorical_crossentropy'], loss_weights=[0.0, 1.0], optimizer='adam', metrics={'Out_SNR': 'mape', 'Out_Bool': 'accuracy'})
    ...
    class CustomDataSet():
        ...
        def load_data(self):
            with h5py.File(self.file_path) as FILE:
                self.signals = FILE['signals']['data'][:125000]
                self.noise = FILE['noise']['data'][:375000]
                self.signal_labels = FILE['signals']['snr'][:125000]
