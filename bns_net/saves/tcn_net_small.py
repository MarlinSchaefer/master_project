...
def get_model():
    ...
    combined = keras.layers.concatenate([incep2, incep8, incep32])
    ...
    model = keras.models.Model(inputs=[is2, in2, is8, in8, is32, in32], outputs=[snr_dense2, bool_dense2, tcn2, tcn8, tcn32])
    
    return(model)

def compile_model(model):
    model.compile(loss=['mse', 'categorical_crossentropy', 'mse', 'mse', 'mse'], optimizer='adam', metrics={'Out_SNR': 'mape', 'Out_bool': 'accuracy'})
        ...
        for i, idx in enumerate(indices):
            ...
            for j in range(self.data_channels):
                ...
                if not sig_ind == -1:
                    ...
                    X[2 * j][i][1] = self.signals[sig_ind].transpose()[ci+self.data_channels]
                    y_3[j][i] = self.signals[sig_ind].transpose()[ci]
                
                X[2 * j + 1][i][0] = self.noise[noi_ind].transpose()[ci]
                X[2 * j + 1][i][1] = self.noise[noi_ind].transpose()[ci+self.data_channels]
