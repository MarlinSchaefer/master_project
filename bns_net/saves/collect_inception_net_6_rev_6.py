...
def incp_lay(x, filter_num, active_filter_sizes=(4, 8, 16), dilation_rate=(1, 1, 1)):
    l = keras.layers.Conv1D(3 * filter_num, active_filter_sizes[0], dilation_rate=dilation_rate[0], padding='same', activation='relu')(x)
    lm_1 = keras.layers.Conv1D(int(round(filter_num / 2.0)), 1, activation='relu')(x)
    lm_2 = keras.layers.Conv1D(2 * filter_num, active_filter_sizes[1], dilation_rate=dilation_rate[1], padding='same', activation='relu')(lm_1)
    rm_1 = keras.layers.Conv1D(int(round(filter_num / 2.0)), 1, activation='relu')(x)
    rm_2 = keras.layers.Conv1D(filter_num, active_filter_sizes[2], dilation_rate=dilation_rate[2], padding='same', activation='relu')(rm_1)
...
def stack(x, NUM_DETECTORS, DROPOUT_RATE):
    ...
    inc_1 = incp_lay(act_conv_2, 32, active_filter_sizes=(4, 4, 4), dilation_rate=(1, 2, 3))
    batch_2 = keras.layers.BatchNormalization()(inc_1)
    inc_2 = incp_lay(batch_2, 32, active_filter_sizes=(4, 4, 4), dilation_rate=(1, 2, 3))
    pool_1 = keras.layers.MaxPooling1D(2)(inc_2)
    batch_3 = keras.layers.BatchNormalization()(pool_1)
    inc_3 = incp_lay(batch_3, 32, active_filter_sizes=(4, 4, 4), dilation_rate=(1, 2, 3))
