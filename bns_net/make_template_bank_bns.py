...
def generate_parameters(num_of_templates, rand_seed, **kwargs):
    exceptions = ['mode_array', 'detectors', 'declination', 'inclination']
    seed(rand_seed)
...
def worker(kwargs):
    ...
    if gw_present:
        ...
        set_temp_offset(strain_list, opt_arg['t_len'], t_offset, opt_arg['t_from_right'])
    ...
    for i, noise in enumerate(noise_list):
        ...
        if isinstance(opt_arg['resample_delta_t'], tuple):
            if isinstance(opt_arg['resample_t_len'], tuple) and len(opt_arg['resample_delta_t']) == len(opt_arg['resample_t_len']):
                rdt = opt_arg['resample_delta_t']
                resample_samples = [int(opt_arg['resample_t_len'][idx] / kwargs['delta_t']) for idx in range(len(rdt))]
                for idx, sam in enumerate(resample_samples):
                    ...
                    #print("")
                    tmp_white.append(resample_to_delta_t(total_white[i][len(total_white[i]) - sam:], rdt[idx]))
            else:
                raise ValueError("The options 'resample_delta_t' and 'resample_t_len' have to either both be floats or tuples of floats of the same length.")
                #Error or handle
        else:
