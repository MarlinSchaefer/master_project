...
if __name__ == "__main__":
    msg = 'The purpose of this network is to try and train the first stage (the TCN part) with respect to mean average percentage error, as its output will be close to one. This in turn means, that a mean squared error of ~1 is on the order of the input and hence the network does not learn a whole lot.'
    run_net('tcn_net_rev_2', 'templates_new', ini_file='testing_net.ini', create_wiki_entry=False, overwrite_template_file=False, epochs=24, use_data_object=True, show_snr_plot=False, overwrite_net_file=True, evaluate_on_large_testing_set=False, batch_size=24, custom_message=msg)
    #try:
