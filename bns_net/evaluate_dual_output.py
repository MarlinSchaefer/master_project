...
def evaluate_dual_output_form(net_name, temp_name, format_name, show=True, screen_name=None):
    """Evaluates the performance of a network with two outputs.
    ...
    _do_plot(net_name if screen_name == None else str(screen_name), np.array(x_pt_1), np.array(x_pt_2), np.array(y_pt), plot_path, show=show)
